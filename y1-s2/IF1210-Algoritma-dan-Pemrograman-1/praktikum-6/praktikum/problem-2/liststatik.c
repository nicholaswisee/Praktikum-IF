...
ListStatik plusMinusList(ListStatik l1, ListStatik l2, boolean plus){
    ...
    for (int i = 0; i < len; i++){
        int result = plus ? (ELMT(l1, i) + ELMT(l2,i)) : (ELMT(l1, i) - ELMT(l2, i));
        
        // SAFETY CHECK: Ensure result doesn't equal MARK
        if (result == MARK) {
            // Optional: set result to a safe placeholder or raise error
            result = MARK + 1; // E.g., if MARK is -9999, use -9998
        }

        ELMT(res, i) = result; 
    }

    ELMT(res, len) = MARK; // Mark proper end of list
    return res; 
}
