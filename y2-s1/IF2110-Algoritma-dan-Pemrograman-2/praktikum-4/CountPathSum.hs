...
countPathSum tree num
    | isTreeEmpty tree = 0
    | isOneElmt (tree) && (akar tree) - num == 0 = 1
    | isOneElmt (tree) && (akar tree) - num < 0 = 0
    | otherwise = (countPathSum (left tree) (num - (akar tree))) + (countPathSum (right tree) (num - (akar tree)))
...
1. Definisikan tree
    let tree = Node 5 (Node 4 (Node 11 (Node 7 Empty Empty) (Node 2 Empty Empty)) Empty) (Node 8 (Node 13 Empty Empty) (Node 4 (Node 5 Empty Empty) (Node 1 Empty Empty)))
