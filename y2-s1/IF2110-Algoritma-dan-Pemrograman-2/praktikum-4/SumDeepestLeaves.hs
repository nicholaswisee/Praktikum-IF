...
treeDepth tree
    | isTreeEmpty tree = 0
    | isOneElmt tree = 1
    | otherwise = 1 + max (treeDepth (left tree)) (treeDepth (right tree))

sumDeepestLeaves :: (BinTree Int) -> Int
sumDeepestLeaves tree
    | isTreeEmpty tree = 0
    | isOneElmt tree = akar (tree)
    | treeDepth (left tree) > treeDepth (right tree) = sumDeepestLeaves(left tree)
    | treeDepth (left tree) < treeDepth (right tree) = sumDeepestLeaves(right tree)
    | otherwise = sumDeepestLeaves (left tree) + sumDeepestLeaves (right tree)
