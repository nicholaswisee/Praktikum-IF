...
class Authenticator {
private:
  User *users[MAX_USERS]; // Array pointer untuk menyimpan objek User atau
                          // AdminUser
  ...
  string secretResource;  // Resource rahasia yang hanya bisa diakses oleh user
                          // yang login

public:
  Authenticator();
  /* Constructor:
   * - Menginisialisasi sistem dengan resource rahasia ("Ini adalah data
   * rahasia!")
   ...
   * - isAdmin: Jika true, user akan dibuat sebagai AdminUser, jika false
   * sebagai User biasa.
   *
   * Output:
   * - "Gagal: Kapasitas user penuh!" jika jumlah user telah mencapai batas
   * maksimum.
   * - "Gagal: Username sudah digunakan!" jika username sudah terdaftar dalam
   * sistem.
   ...
   *
   * Output:
   * - "Gagal: Username atau password salah!" jika kombinasi username dan
   * password tidak ditemukan.
   * - "Sukses: Login berhasil! Selamat datang, <username>." jika login
   * berhasil.
   ...
   *
   * Output:
   * - "Gagal: Tidak ada user yang sedang login!" jika tidak ada user yang
   * sedang login.
   ...
   *
   * Output:
   * - "Akses ditolak! Silakan login terlebih dahulu." jika tidak ada user yang
   * login.
   ...
   * - "Gagal: Tidak bisa mengubah resource! Hanya admin yang dapat melakukan
   * ini." jika user yang login bukan AdminUser.
   * - "Sukses: Resource telah diperbarui oleh <username>." jika resource
   * berhasil diubah.
   *      *<username> diganti dengan nama pengguna admin yang melakukan
   * perubahan.
   */
