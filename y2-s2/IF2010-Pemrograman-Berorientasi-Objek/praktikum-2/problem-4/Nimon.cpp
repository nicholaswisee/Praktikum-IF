...
int Nimon::getLoyalty() const { return loyalty; }

std::string Nimon::getProfileLine() const {
  return name + " | PWR " + std::to_string(power) + " | DIM " + dimension +
         " | WORK " + std::to_string(getWorkHours()) + "h-" +
         getSpecialSkill() + " | DEF " + std::to_string(getArmor()) + "-" +
         getWeapon() + " | LOY " + std::to_string(getLoyalty());
}
